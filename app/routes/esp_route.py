...
@router.get("/idle", response_model=list[ESPPublic])
def get_esps(current_user: Annotated[User, Depends(get_current_user)], session: SessionDep):
    statement = select(ESP).where(ESP.user_id == current_user.id).where(ESP.status == Status.IDLE)
    esps = session.exec(statement).all()
    return esps

@router.get("/running", response_model=list[ESPPublic])
def get_esps(current_user: Annotated[User, Depends(get_current_user)], session: SessionDep):
    statement = select(ESP).where(ESP.user_id == current_user.id).where(ESP.status == Status.RUNNING)
    esps = session.exec(statement).all()
    return esps
